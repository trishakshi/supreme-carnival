...
router.post("/register", async (req, res) => {
  try {
    ...
    if (password.length < 6) {
      return res
        .status(400)
        .json({ err: "Password must be six characters long." });
    }
    ...
    const token = jwt.sign(
      { registeredToken: { id: savedUser._id, roleId: savedUser.roleId } },
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!existingUser) {
      return res
        .status(400)
        .json({ err: "Account doesn't exists. Please register first." });
    }
    ...
    const token = jwt.sign(
      {
        registeredToken: { id: existingUser._id, roleId: existingUser.roleId },
      },
      process.env.JWT_SECRET
    );
    existingUser.token = token;
    res.cookie("token", token, { httpOnly: true }).send(existingUser);
  } catch (err) {
    return res.status(500).send();
  }
});
